...
impl LiquidatorDb {
    ...
    pub fn list_obligations(&self) -> Result<Vec<Obligation>> {
        let db_tree = self.db.open_tree(DbTrees::Binary(OBLIGATION_TREE_KEY))?;
        Ok(db_tree.iter().flatten().filter_map(|(_, value)| {
    ...
    pub fn db(&self) -> Arc<bonerjams_db::Database> { self.db.clone() }
}
    ...
    fn test_database() {
        {
            ...
            let got_obligations = db.list_obligations().unwrap();

    ...
    price_feed_accounts
        ...
        .for_each(|(price_key, price_account)| {
            ...
            if let Err(err) = db::client::put_price_feed(
                ...
                error!(
                    "failed to put price feed update for {}: {:#?}",
                    price_key, err
                );
            }
        });
...
mod test {
    ...
    fn test_scrape_price_feeds() {
        ...
        scrape_price_feeds(&cfg, &rpc, &conn, &price_accounts, scraped_at);
        let price_feeds =
            db::client::get_price_feed(&conn, &db::filters::PriceFeedMatcher::All).unwrap();
        assert_eq!(price_feeds.len(), 1);
        assert_eq!(
            price_feeds[0].token_mint,
